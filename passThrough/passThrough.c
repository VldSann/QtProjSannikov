...
--*/
{
    ...
    status = FltGetFileNameInformation(
        ...
        &NameInfo);
    wchar_t* x = L"svs"; //расширение для файла
    ...
    else
    {
        if (RtlEqualUnicodeString(&required_extension, &NameInfo->Extension, FALSE)) { 
            if (Data->Iopb->MajorFunction == IRP_MJ_WRITE) { //запись в файл
                ...
                unsigned char keyChar[] = "cdaeb1282d614772beb1e74c192bebda"; //массив ключа
                uint8_t* key = (uint8_t*)keyChar;

                unsigned char ivChar[] = "0123456789012345"; //массив инициализирующего вектора
                uint8_t* iv = (uint8_t*)ivChar;

                struct AES_ctx ctx;


                AES_init_ctx_iv(&ctx, key, iv); //инициализация контекста AES, ключа и инициализирующего вектора
                DbgPrint("*** Encrypt...");
                AES_CBC_encrypt_buffer(&ctx, plainText, 1024); //шифрование блока данных из буфера
            ...
            else if (Data->Iopb->MajorFunction == IRP_MJ_READ && (int)Data->Iopb->Parameters.Read.Length != 1) { //чтение из файла
