...
static sgx_status_t SGX_CDECL sgx_get_elem(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_get_elem_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_get_elem_t* ms = SGX_CAST(ms_get_elem_t*, pms);
	ms_get_elem_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_get_elem_t), ms, sizeof(ms_get_elem_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	char* _tmp_buffer = __in_ms.ms_buffer;
	int _tmp_len = __in_ms.ms_len;
	...
	get_elem(_in_buffer, _tmp_len, __in_ms.ms_index);
	if (_in_buffer) {
...
SGX_EXTERNC const struct {
	...
	1,
	{
		{(void*)(uintptr_t)sgx_get_elem, 0, 0},
	}
};
